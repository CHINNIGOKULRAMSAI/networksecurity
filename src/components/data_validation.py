...
class DataValidation:
    ...
    def validate_no_of_columns(self, dataframe: pd.DataFrame):
        try:
            number_no_of_columns = len(self._schema_config)

            logging.info(f"Required no of columns {number_no_of_columns}")
            logging.info(f"DataFrame columns {len(dataframe.columns)}")

            if len(dataframe.columns) == number_no_of_columns:
                return True
            return False
        except Exception as e:
            raise NetworkSecurityException(e, sys)
    ...
    def detect_dataset_drift(self, base_df, current_df, threshold = 0.05) -> bool:
        try:
            ...
            for column in base_df.columns:
                ...
                is_same_distnt = ks_2samp(d1, d2)
                if threshold <= is_same_distnt.pvalue:
                    is_found = False
                else:
                    is_found = True
                    status = False
                report.update({column:{
                    "p_value" : float(is_same_distnt.pvalue),
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        try:
            ...
            train_dataframe = DataValidation.read_data(self,file_path=train_file_path)
            test_dataframe = DataValidation.read_data(self,file_path=test_file_path)
